...
from sandbox.young_clgan.envs.maze.maze_env_utils import sample_nearby_states
...
def compute_rewards_from_paths(all_paths, key='rewards', as_goal=True, env=None, terminal_eps=0.1):
    """if key=competence, env needs to be a GoalExplorationEnv!"""
    all_rewards = []
    all_states = []
    for paths in all_paths:
        for path in paths:
            if key == 'competence':
                ...
                start_state = np.array(tuple(path['env_infos']['obs2goal'][0]))
                end_state = np.array(tuple(path['env_infos']['obs2goal'][-1]))
...
def label_states_from_paths(all_paths, min_reward=0, max_reward=1, key='rewards', as_goal=True,
                 old_rewards=None, improvement_threshold=0, n_traj=1, env=None, return_mean_rewards = False,
                            order_of_states = None, one_layer=False):
    state_dict = {}
    if one_layer:
        all_paths = [all_paths]
    for paths in all_paths:
...
def label_states(states, env, policy, horizon, as_goals=True, min_reward=0.1, max_reward=0.9, key='rewards',
                 ...
                 using_gym=False, noise=0, aggregator=(np.sum, np.mean)):
    logger.log("Labelling starts")
    result = evaluate_states(
        states, env, policy, horizon, as_goals=as_goals,
        n_traj=n_traj, n_processes=n_processes, key=key, full_path=full_path, using_gym=using_gym, noise=noise, aggregator=aggregator
    )
    if full_path:
    ...
    print("mean rewards", np.mean(mean_rewards))
...
def evaluate_states(states, env, policy, horizon, n_traj=1, n_processes=-1, full_path=False, key='rewards',
                    ...
                    using_gym=False, noise=0, feasible_states=None, test_with_nearby=True, plan=None):
    evaluate_state_wrapper = FunctionWrapper(
        ...
        test_with_nearby=test_with_nearby,
        plan=plan
    )
...
def evaluate_state(state, env, policy, horizon, n_traj=1, full_path=False, key='rewards', as_goals=True,
                   aggregator=(np.sum, np.mean), using_gym=False, noise=0, feasible_states=None, test_with_nearby=True, plan=None, animated=False):
    aggregated_data = []
    paths = []
    if as_goals:
    ...
    if feasible_states is not None:
        if test_with_nearby:
            sampled_states = sample_nearby_states(env, state, feasible_states, n=3, num_samples=n_traj)
        else:
            sampled_states = np.array(feasible_states)[np.random.choice(len(feasible_states), size=n_traj)]
        for goal in sampled_states:
            if env.relative_goal:
                env.update_goal_generator(FixedStateGenerator(goal - np.array(state)))  # relative goals!
            else:
                env.update_goal_generator(FixedStateGenerator(goal))
            path = rollout(env, policy, horizon, using_gym=using_gym, noise=noise)
            paths.append(path)
        ...
        for j in range(n_traj):
            paths.append(rollout(env, policy, horizon, using_gym=using_gym, noise=noise, plan=plan, animated=animated, speedup= 3))
