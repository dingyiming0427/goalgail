...
from sandbox.young_clgan.envs.pick_n_place.pick_n_place_gym import PnPEnv
...
def run_task(v):
    ...
    from sandbox.young_clgan.experiments.goals.pick_n_place.pnp_expert import PnPExpert
    ...
    buffer_shapes['g'] = (buffer_shapes['g'][0], 3 if not v['full_space_as_goal'] else 6)
    buffer_shapes['ag'] = (v['horizon'] + 1, 3 if not v['full_space_as_goal'] else 6)
    ...
    for _ in range(v['num_demos']):
        ...
        episode = expert_worker.generate_rollouts(slice_goal=(3, 6) if v['full_space_as_goal'] else None)
