...
class Discriminator(tf.keras.Model):
  ...
  def __init__(self, input_dim):
    ...
    self.main = tf.keras.Sequential([
        ...
        tf.layers.Dense(
            units=256, activation=tf.nn.tanh, kernel_initializer=kernel_init),
...
class GAIL(object):
  ...
  def update(self, batch, expert_batch):
    ...
    tf.get_default_session().run(self.update_ops, feed_dict={self.obs: batch['o'],
                                                             self.expert_obs: expert_batch['o'],
                                                             self.action: batch['u'] if not self.use_s_p else batch['o_2'],
                                                             self.expert_action: expert_batch['u'] if not self.use_s_p else expert_batch['o_2'],
                                                             self.goal: batch['g'],
                                                             self.expert_goal: expert_batch['g'],
                                                             self.alpha: np.random.uniform(size=(batch['o'].shape[0], 1))})


  def get_reward(self, obs, action, goal, whole_batch=None):  # pylint: disable=unused-argument
    ...
    if obs == None:
      obs = whole_batch['o']
      action = whole_batch['u'] if not self.use_s_p else whole_batch['o_2']
      goal = whole_batch['g']
    if self.gail_loss == 'airl':
      return tf.get_default_session().run(self.airl_rew, {self.obs: obs, self.action: action, self.goal:goal})
    elif self.gail_loss == 'gail':
      return tf.get_default_session().run(self.gail_rew, {self.obs: obs, self.action: action, self.goal: goal})
    elif self.gail_loss == 'normalized':
      return tf.get_default_session().run(self.normalized_rew, {self.obs: obs, self.action: action, self.goal: goal})
    elif self.gail_loss == 'negative':
      return tf.get_default_session().run(self.negative_rew, {self.obs: obs, self.action: action, self.goal: goal})
