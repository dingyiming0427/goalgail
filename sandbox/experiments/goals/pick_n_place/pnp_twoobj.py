...
from sandbox.experiments.goals.pick_n_place.pnp_twoobj_algo import run_task
...
if __name__ == '__main__':
    ...
    vg.add('extend_distance_rew', [False]) # TODO: figure out why no matter what we put here it's going to be 0.
    ...
    vg.add("to_goal", lambda full_space_as_goal: [(3, 4, 5, 6, 7, 8)] if not full_space_as_goal else [(0, 1, 2, 3, 4, 5, 6, 7, 8)])

    vg.add('horizon', [150])
    vg.add('outer_iters', [3000])
    vg.add('inner_iters', [5])

    vg.add('n_evaluation_traj', [3])

    vg.add('num_demos', lambda mode: [0] if mode == 'her' else [20])
    vg.add('relabel_expert', lambda num_demos: [False] if num_demos > 0 else [False])
    ...
    vg.add('use_s_p', lambda mode: [True] if 'gail' in mode else [False])
    vg.add('only_s', lambda use_s_p: [False] if use_s_p else [False])
