...
class FetchEnv(robot_env.RobotEnv):
    ...
    def _get_obs(self):
        ...
        if not self.has_object:
            ...
            achieved_goal = np.concatenate([np.squeeze(object_pos.copy()), np.squeeze(object_pos1.copy())])
        obs = np.concatenate([
            grip_pos, object_pos.ravel(), object_pos1.ravel(), object_rel_pos.ravel(), object_rel_pos1.ravel(),
            gripper_state, object_rot.ravel(), object_velp.ravel(), object_velr.ravel(), grip_velp, gripper_vel,
        ])
    ...
    def _sample_goal(self):
        if self.has_object:
            ...
            if not self.stacking:
                # second object can be anywhere
                goal1 = self.initial_gripper_xpos[:3] + self.np_random.uniform(-self.target_range, self.target_range, size=3)
                ...
                goal1 = np.copy(goal0)
                goal1[2] = goal0[2] + 0.025 # TODO: figure out if it's 0.025 or 0.05
