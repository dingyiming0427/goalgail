...
from sandbox.envs.base import StateGenerator, UniformListStateGenerator, \
    UniformStateGenerator, FixedStateGenerator, StateAuxiliaryEnv
from sandbox.state.evaluator import parallel_map, FunctionWrapper
from sandbox.state.utils import StateCollection
from sandbox.logging.visualization import plot_labeled_states, plot_labeled_samples
from sandbox.state.evaluator import FunctionWrapper, parallel_map
...
def generate_starts(env, policy=None, starts=None, horizon=50, size=10000, subsample=None,
                    zero_action=False, animated=False, speedup=1):
    """ If policy is None, brownian motion applied """
    if starts is None or len(starts) == 0:
        starts = [env.transform_to_start_space(env.reset())]
    ...
    if horizon <= 1:
        ...
        while len(states) < size:
            if animated:
                ...
                if done or steps >= horizon:
                    ...
                    # import pdb; pdb.set_trace()
                    states.append(env.start_observation)
                    if done and env_info[
                        'goal_reached']:  # we don't care about goal done, otherwise will never advance!
                        goal_reached = True
                        done = False
                        env.render()
                        timestep = 0.05
                ...
                logger.log("parallel num:" + str(singleton_pool.n_parallel))
        ...
        logger.log(
            "Generating starts, rollouts that reached goal: " + str(num_roll_reached_goal) + " out of " + str(num_roll))
    logger.log("Starts generated.")
    states = np.stack([np.array(state) for state in states])
    if subsample is None:
        return states
    else:
        # states = np.stack([np.array(state[:2]) for state in states]) #modified for ddpg
        if len(states) < subsample:
            return states
        return states[np.random.choice(np.shape(states)[0], size=subsample)]
...
def find_all_feasible_states_plotting(env, seed_starts, report, distance_threshold=0.1, size=10000, horizon=300,
                                      brownian_variance=1, animate=False,
    ...
    while no_new_states < 30:
        ...
        new_starts = generate_starts(env, starts=starts, horizon=horizon, size=size,
        ...
        if check_feasible:  # used for ant maze environment, where we ant to run no_action
            ...
            new_starts = parallel_check_feasibility(env=env, starts=new_starts,
        ...
        img = plot_labeled_samples(samples=np.concatenate((added_states[:, :2], all_starts_samples[:, :2]), axis=0),
                                   # first two are COM
                                   sample_classes=np.concatenate(
                                       (np.zeros(num_new_starts, dtype=int), np.ones(num_samples, dtype=int)), axis=0),
                                   text_labels=text_labels,
                                   limit=limit,
                                   center=center,
                                   maze_id=0,
...
def brownian(start, env, horizon, policy=None):
    ...
    while not done and steps < horizon:
        ...
        obs, _, done, env_info = env.step(action)
        states.append(env.start_observation)
        if done and 'goal_reached' in env_info and env_info[
            'goal_reached']:  # we don't care about goal done, otherwise will never advance!
            goal_reached = True
            done = False
            # print(obs, goal_reached, done)
...
def find_all_feasible_states(env, seed_starts, distance_threshold=0.1, brownian_variance=1, animate=False, speedup=10,
    ...
    while no_new_states < 5:
        ...
        new_starts = generate_starts(env, starts=starts, horizon=horizon, size=10000,
...
def find_all_feasible_reject_states(env, distance_threshold=0.1, ):
    ...
    uniform_state_generator = UniformStateGenerator(state_size=len(env.current_start),
                                                    bounds=env.start_generator.bounds)
