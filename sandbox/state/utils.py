...
import numpy as np
import scipy.misc
from sandbox.state.evaluator import parallel_map, disable_cuda_initializer
...
class StateCollection(object):
    ...
    def append(self, states, n_process=None):
        ...
        if len(states) > 0:
            ...
            if n_process is None:
                ...
                n_process = 1
            if n_process > 1 and states.shape[0] > 2 * n_process:  # only us parallel if enough states left to append
    ...
    def _select_states(self, states):
        """append to the state list only the ones that are further away from the others"""
